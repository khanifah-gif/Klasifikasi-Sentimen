...
#Translate emoticon
emoticon_df = pd.read_csv('D:/Users/user/Documents/FRESH GRADUATED/Atmatech/DEPLOYMENT/emoticon.txt', sep='\t', header=None)
...
def preprocess_text(dataframe, idf):
    dataframe['original_text'] = dataframe['original_text'].apply(lambda text: remove_pattern(text, r"@[\w]*"))
    dataframe['original_text'] = dataframe['original_text'].apply(remove_emoji)
    dataframe['original_text'] = dataframe['original_text'].apply(translate_emoticon)
    dataframe['original_text'] = dataframe['original_text'].apply(remove_kaskus_formatting)
    dataframe['original_text'] = dataframe['original_text'].apply(clean_caracter)
    dataframe['original_text'] = dataframe['original_text'].apply(transform_slang_words)
    dataframe['original_text'] = dataframe['original_text'].apply(remove_url)
    dataframe['original_text'] = dataframe['original_text'].apply(remove_whitespace)
    dataframe['original_text'] = dataframe['original_text'].apply(remove_non_alphabet)
    dataframe['original_text'] = dataframe['original_text'].apply(remove_repeating_characters)
    dataframe['original_text'] = dataframe['original_text'].apply(tokenizing)

    # Melakukan stemming pada kolom 'original_text'
    term_base = {}
    for document in dataframe['original_text']:
        for term in document.split():
            if term not in term_base:
    ...
    dataframe['original_text'] = dataframe['original_text'].apply(lambda x: ' '.join([term_base[word] for word in x.split()]))
    idf = calculate_idf(dataframe['original_text'])
    dataframe['original_text'] = dataframe['original_text'].apply(lambda text: calculate_tfidf(text, idf))
...
class SentimentAnalysis(Resource):
    ...
    def post(self):
        ...
        processed_text = np.zeros((len(preprocessed_df), max_features))
...
class BatchSentimentAnalysis(Resource):
    ...
    def post(self):
        ...
        df2 = pd.DataFrame({'original_text': texts})

        # Lakukan preprocessing untuk setiap teks dalam batch
        preprocessed_df = preprocess_text(df2)

        processed_texts = np.zeros((len(preprocessed_df), max_features))
        for i, tfidf_list in enumerate(preprocessed_df['original_text']):
